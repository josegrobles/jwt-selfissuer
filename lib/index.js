...
let handler = (key, verification_options={ignoreExpiration:true}, signing_options={}, redisClient) => {
    return (req, res, next) => {
        if ((token = isValid(req.headers.authorization)) !== null) {
          verify(token.value,key,req,verification_options, signing_options).then((result) =>{
