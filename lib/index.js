...
let handler = (key, redisClient, verification_options={ignoreExpiration:true}, signing_options={expiresIn:900}) => {
    return (req, res, next) => {
        if ((token = isValid(req.headers.authorization)) !== null) {
          verify(token.value, key, req, redisClient,verification_options, signing_options).then((result) =>{
            ...
            if (err) res.status(400).end(JSON.stringify({status:"error",message:err.message,error_type:err.name}))
