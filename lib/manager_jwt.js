...
let verifier_jwt = (token, key, req, verification_options, signing_options) => {
  return new Promise((resolve,reject) => {
      jwt.verify(token, key, verification_options, (err,decoded) => {
        ...
        try{
          ...
          req.info = info
          check_expire(decoded)
          ?
          (
            issuer_jwt(info, key ,signing_options).then((result) => { resolve(result) }).catch((err) => { reject(err) })
          )
          : resolve(token)
        }catch(err){
          reject(err)
...
let issuer_jwt = (info, key, options) => {
  return new Promise((resolve,reject) => {
    ...
    info.exp = expire
    resolve(jwt.sign(info, key, options))
  })
}
